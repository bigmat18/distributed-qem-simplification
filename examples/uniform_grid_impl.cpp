...
int main(int argc, char **argv) {
...

    {
        ...
        //LOG_DEBUG("Parallel computation mesh vertices: {}, edges: {}, faces: {}", mesh.n_vertices(), mesh.n_edges(), mesh.n_faces());
        //{
            //PROFILING_SCOPE("Refinements");
            //std::vector<qems::QEMMesh::EdgeHandle> elements;

            //#pragma ompo parallel
            //{
                //#pragma omp for schedule(static) 
                //for (size_t i = 0; i < mesh.n_vertices(); ++i) {
                    //auto vh = qems::QEMMesh::VertexHandle(i);
            ...
            //qems::QEMPriorityQueue pq(qems::QEMEdgeCompare(mesh), std::move(elements));
            //qems::simplification(mesh, TARGET_FACES, mesh.n_faces(), pq);
            //mesh.garbage_collection();
        //}
    }   

    LOG_DEBUG("Final computation mesh vertices: {}, edges: {}, faces: {}", mesh.n_vertices(), mesh.n_edges(), mesh.n_faces());
    massert(OpenMesh::IO::write_mesh(mesh, "out/ug.ply"), "Error in mesh export!");
